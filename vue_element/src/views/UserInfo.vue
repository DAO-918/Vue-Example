        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    ...
                    <el-button type="danger" @click="delUser(scope.row.id, scope.row.name)" size="mini"><i class="el-icon-delete"></i> 删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-pagination
        ...
        <el-dialog title="添加用户" :visible.sync="showAddUserDialog">
            ...
            <el-form ref="userModel" :model="userModel" :rules="userRules" label-width="80px">
                ...
                <el-form-item label="生日" prop="birthday">
                    <el-date-picker type="date" v-model="userModel.birthday" value-format="yyyy-MM-dd"></el-date-picker>
                </el-form-item>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                //[{id: 1, name: 'XX', age: 20, email: 'aa@163.com'}, .....]
                users: [],
                total: 0,
                currentPage: 1,
                ...
                userRules: {
                    ...
                    email: [{required: true, message: '邮箱不能为空', trigger: 'blur'},
                        {pattern: /^\w[-\w]{2,15}@[a-zA-Z0-9]{2,6}\.[a-zA-Z]{2,4}$/, message: '请填写正确的邮箱格式', trigger: 'blur'}],
        ...
        methods: {
            //row指一行的数据, column指一列的数据，cellValue指的是格子的数据
            ...
            toPage(val) {
                ...
                this.axios.get('/user?page=' + val + "&size=10")
                    .then(resp => {
                        //resp.data才是实际的数据本身
            ...
            //添加数据
            addUser() {
                window.console.log(this.userModel.birthday);
                //valid的值 true或者false
                this.$refs.userModel.validate(valid => {
                    if(valid) {
                        this.axios.post('/user', this.userModel)
                            .then(resp => {
                                let code = resp.data.code;
                                if(code > 0) {
